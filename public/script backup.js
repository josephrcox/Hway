...
const postObject = {
    ...
    display() {
        ...
        voteCount.innerHTML = this.votes
        ...
        infoCell.innerHTML = "Submitted by "+getUserColor(this.poster)+this.poster+"</span> in "+"<span style='color:blue; font-weight: 900;'><a href='javascript:loadPosts(0,"+"\""+this.topic+"\""+",1);'>"+this.topic+"</a></span>  on " +this.datetime
        ...
        if (this.description != "(empty)") {
            descCell.innerHTML = this.description
            descCell.style.display = 'none'
            titleCell.innerHTML = this.title+"<span style='font-size:12px'>        (+)</span>"  
        } 
...
const commentObject = {
    ...
    display() {
        ...
        infoCell.innerHTML = this.datetime
...
const loadPosts = async (x, topic, page) => {
    ...
    if (topic == "" || topic == null) {
        ...
        const response = await fetch('/all/'+sorting+'/'+page)
    ...
    for (i=0; i<data.length ; i++) {
        ...
        if (post.description == "" || post.description == undefined || post.description == null) {
...
function getComment(commentid, type) {
    index = comments.indexOf(commentid)
...
const loadComments = async (postid) => {
    ...
    for (i=0; i<data.length ; i++) {
        if (data[i].body == null && data[i].body == "") {
            ...
            com.votes = data[i].votes
            com.upvotes = data[i].upvotes
            com.downvotes = data[i].downvotes
            com.poster = data[i].poster
            com.datetime = data[i].datetime

            comments.push(com.id)
...
const storeAndDisplayTopics = async () => {
    ...
    const response = await fetch('/admin/topicsinfo/')
    var data = await response.json()
...
const voteCom = async (d, id) => { 
    ...
    index = comments.indexOf(id.substring(13))
...
const comment = async (x, y, z, uvo) => { 
    if (y != null && y != "") {
        ...
        com.id = data._id
        com.pid = data.parent
        com.votes = data.votes
        com.upvotes = data.upvotes
        com.downvotes = data.downvotes
            ...
            if (postTitle == "" || postTitle == null) {
                ...
                if (usernameArray.includes(poster)) {
                    ...
                    if (localStorage.getItem(idArray[index]) == "true") {
                        postText(postTitle, postDesc, poster, true, topic)
                    } else {
                        postText(postTitle, postDesc, poster, false, topic)
                    }
                } else {
                    postText(postTitle, postDesc, poster, true, topic)
                }
...
function postRandomly() {
    ...
    descriptionBoolean = Math.floor(Math.random() * 2) // 0 if no, 1 if yes
    descriptionLengthInWords = Math.floor(Math.random() * 200)
    titleStr = ""
    posterStr = ""
    descriptionStr = ""
    topicStr = ""
    ...
    if (descriptionBoolean == 1 || descriptionBoolean == "1") {
        for (i=0;i<descriptionLengthInWords;i++) {
            descriptionStr += englishWordsArrayForRandomization[Math.floor(Math.random() * 1000)]+" "
        }
    } else {
        descriptionStr = ""
    }
    ...
    postText(titleStr, descriptionStr, posterStr, true, topicStr)
}
